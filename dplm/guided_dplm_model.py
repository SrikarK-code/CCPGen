...
from diffmap_classifiers import MLPClassifier, TransformerClassifier

@register_model('guided_cond_dplm')
class GuidedConditionalDPLM(nn.Module):
    def __init__(self, cfg):
        ...
        # Initialize the DiffMap classifier based on the configuration
        if cfg.classifier_type == 'mlp':
            self.diffmap_classifier = MLPClassifier(
                input_dim=cfg.diffmap_dim,
                hidden_dims=cfg.classifier_hidden_dims,
                output_dim=cfg.encoder.d_model,
                dropout=cfg.classifier_dropout
            )
        elif cfg.classifier_type == 'transformer':
            self.diffmap_classifier = TransformerClassifier(
                input_dim=cfg.diffmap_dim,
                hidden_dim=cfg.classifier_hidden_dim,
                output_dim=cfg.encoder.d_model,
                num_layers=cfg.classifier_num_layers,
                num_heads=cfg.classifier_num_heads,
                dropout=cfg.classifier_dropout
            )
        else:
    ...
    def forward_encoder(self, batch, use_draft_seq=False):
        ...
        # Use the DiffMap classifier to guide the encoding
        diffmap_embedding = self.diffmap_classifier(batch['X_diffmap'])
    ...
    def generate(self, batch, tokenizer=None, 
        ...
        for _ in range(num_samples):
            ...
            results.append(prev_decoder_out['output_tokens'])
